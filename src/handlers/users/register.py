...
async def register_command(message: Message, state: FSMContext, bot: Bot, repo: RequestsRepo):
    try:     
        ...
        if user.is_registered:
            await message.answer(text="👋 Botga hush kelibsiz!\nQuyidagi o'zingizga kerakli tugmani bosing", reply_markup=admin_base_menu_keyboards if message.from_user.id in config.tg_bot.admin_ids else base_menu_keyboards)
        else:
...
async def register_phone_form(message: Message, state: FSMContext, repo: RequestsRepo, config: Config):
    try:
