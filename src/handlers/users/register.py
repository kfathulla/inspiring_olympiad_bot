...
from src.keyboards.inline.base_menu import base_menu_keyboards, admin_base_menu_keyboards

register_router = Router()

@register_router.message(PrivateFilter(), Command("register"))
async def register_command(message: Message, state: FSMContext, config: Config, bot: Bot, repo: RequestsRepo):
    try:     
        user = await repo.users.get_by_id(message.from_user.id)
        if user.is_registered:
            await message.answer(text="👋 Botga hush kelibsiz!\nQuyidagi o'zingizga kerakli tugmani bosing", reply_markup=admin_base_menu_keyboards(user.private_channel_link) if message.from_user.id in config.tg_bot.admin_ids else base_menu_keyboards(user.private_channel_link))
        else:
...
async def register_phone_form(message: Message, state: FSMContext, user: User, repo: RequestsRepo, config: Config):
    try:
        data = await state.update_data(phone=message.contact.phone_number.strip('+'))
        await repo.users.update_user(id=message.from_user.id, full_name=data["fullname"], phone=data["phone"], is_registered=True, private_channel_link=user.private_channel_link)
        await state.clear()
